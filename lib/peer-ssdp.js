...
var Peer = function (options) {
    this.mcSocket = null;
    this.ucSocket = null;
    this.port = options.port || SSDP_PORT;
    this.host = SSDP_ADDRESS + ":" + this.port;
};
...
Peer.prototype.start = function () {
    ...
    var socketHandling = function (adr, port) {
        socketMap[adr] = {
            unicast: null,
            multicast: null
        };
        ...
        mc.on('error', onError);
        mc.on('close', onClose);
        mc.bind(port, onBind(mc, this, adr, true));
    ...
    for (var i in interfaces) {
        for (var j = 0; j < interfaces[i].length; j++) {
            ...
            if (intr.family == 'IPv4' && !intr.internal) {
                socketHandling(intr.address, this.port);
            }
        }
    }
...
Peer.prototype.notify = function (headers, callback) {
    headers['HOST'] = headers['HOST'] || this.host;
    ...
    var processMessageCallback = function (networkInterfaceAddress) {
        ...
        return [msg, 0, msg.length, this.port, SSDP_ADDRESS, function (err, bytes) {
